...
TOKEN = "7186315620:AAFVqu98eT-7ivNGaTZVxeQgaXg8NeGxM_M"
...
def handle_spam(message):
    ...
    for player_id in player_ids:
        for i in range(number_of_invites):
            api_url = f"https://mr3skr-api.onrender.com/request?api_key=55&uid={player_id}"
            try:
        ...
        info_api_url = f"https://mr-3skr-api-info-v1-1-7jxy.onrender.com/{player_id}?key=1"
        try:
...
def handle_like(message):
    ...
    api_url = f"https://likes-new-mr-3skr1.onrender.com/like?uid={player_id}&key=555"
    try:
        response = requests.get(api_url)
        if response.status_code == 200:
            data = response.json()
...
def handle_visit(message):
    ...
    player_id = parts[1]
    api_url = f"https://mr-3skr-api-y.onrender.com/visit/uid={player_id}"
    try:
        response = requests.get(api_url)
        if response.status_code == 200:
...
def handle_search(message):
    ...
    name = parts[1]
    api_url = f"https://mr-3skr-api-ofc-meqr.onrender.com/search?name={name}"
    try:
        response = requests.get(api_url)
        if response.status_code == 200:
            data = response.json()
